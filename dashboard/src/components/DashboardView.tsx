    ...
    <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans antialiased selection:bg-emerald-500/30 selection:text-emerald-300">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-16 bg-slate-950/95 z-40 p-6 flex flex-col gap-3 font-mono text-xs pt-12">
          <button onClick={() => { setActiveTab('overview'); setIsMobileMenuOpen(false); }} className="p-4 border border-slate-800 rounded bg-slate-900 text-left">📊 METRICS OVERVIEW</button>
          <button onClick={() => { setActiveTab('tasks'); setIsMobileMenuOpen(false); }} className="p-4 border border-slate-800 rounded bg-slate-900 text-left">📋 SLACK REGISTRY</button>
          <button onClick={() => { setActiveTab('archive'); setIsMobileMenuOpen(false); }} className="p-4 border border-slate-800 rounded bg-slate-900 text-left">🗑️ ARCHIVE VAULT ({archivedTasks.length})</button>
          <button onClick={() => { setActiveTab('profile'); setIsMobileMenuOpen(false); }} className="p-4 border border-slate-800 rounded bg-slate-900 text-left">👤 SYSTEM OPERATOR</button>
        </div>
      )}
